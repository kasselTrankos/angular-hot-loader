...
if (window.__angularComponentProxies) {
  componentProxies = window.__angularComponentProxies;
} else {
  componentProxies = {};
  Object.defineProperty(window, '__angularComponentProxies', {
    configurable: true,
    enumerable: false,
    writable: false,
...
export default function proxyAngularComponents({ filename, components, imports, locals }) {
  console.log('filenmae:',filename, 'components:', components, 'imports:', imports,, 'locals', locals, 'loals');
  /*const [React] = imports;
  const [{ hot }] = locals;
